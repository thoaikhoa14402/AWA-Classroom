...
const ReviewRequestForm: React.FC<ReviewRequestFormProps> = ({ details }) => {
    ...
    const handleOpenReview = () => {
        ...
        axios.post(`${process.env.REACT_APP_BACKEND_HOST}/v1/review/${classID}`, {
            reason,
            expected,
            grade,
            composition,
            ...
            headers: {
                Authorization: `Bearer ${authStorage.isLogin() ? authStorage.getAccessToken() : ''}`,
            },
        }).then(res => {
            dispatch(pushReview(res.data.data.review));
            messageApi.success('Open review request successfully !', 1.5, () => {
                navigate(`/classes/reviews/${classID}/view/${res.data.data.review._id}`);
            });
        }).catch(error => {
            ...
            <div className='p-3 flex gap-8 items-start'>
                ...
                <div className='flex-1 flex flex-col gap-3'>
                    <div className='flex items-stretch justify-between'>
                        ...
                        <Button onClick={handleOpenReview} type='primary' className='!py-3 !px-4 !h-auto !font-medium' icon={<FontAwesomeIcon icon={faBullhorn} />}>Open Request</Button>
                    </div>
                    <Editor
